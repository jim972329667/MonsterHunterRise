...
namespace Monster_Hunter_Rise
{
	class SaveData
	{
		...
		public uint FindAddress(byte[] Header)
		{
			uint num = 0;
			if (mBuffer == null) return num;
			if (Header == null || Header.Length < 9) return num;

			for (uint i = 0; i < mBuffer.Length - Header.Length + 1; i++)
			{
				if(mBuffer[i] == Header[0])
                {
					if(mBuffer[i + 8] == Header[8])
                    {
                        if (mBuffer[i + Header.Length - 1] == Header[Header.Length - 1])
                        {
							byte[] Check = mBuffer.Skip((int)i).Take(Header.Length).ToArray();
							if (Check.SequenceEqual(Header))
                            {
								num = i + (uint)Header.Length;
                            }
                        }
                    }
                }
